  ...
  .then(response => {
    ...
    const buildPost = (_element) => {
      ...
      if(getSavedPosts.length >= 1) {
        ...
        if (_element.id == postId) {
          ...
          post.innerHTML = `
            <button id="${_element.id}" class="postControlls">Quit Idea 💾️</button>
            <p class="body">${_element.postContent}</p>
            <p class="author">${_element.postSignature} on ${_element.date}</p>
          `;
        } else {
          post.innerHTML = `
            <button id="${_element.id}" class="postControlls">Save Idea 💾️</button>
            <p class="body">${_element.postContent}</p>
            <p class="author">${_element.postSignature} on ${_element.date}</p>
          `;
        }
    ...
    function showData(_data) {
      ...
      if(_data == '') {
        ...
        noPostsMessage.classList.add('post__messages');
        noPostsMessage.innerHTML = `No hay registros 🤧️`;
